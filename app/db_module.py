...
STATES = {'AK': 'Alaska',  'AL': 'Alabama', 'AR': 'Arkansas', 'AZ': 'Arizona', 'CA': 'California', 'CO': 'Colorado',  'CT': 'Connecticut', 'DE': 'Delaware', 'FL': 'Florida',  'GA': 'Georgia', 'HI': 'Hawaii', 'IA': 'Iowa', 'ID': 'Idaho', 'IL': 'Illinois', 'IN': 'Indiana', 'KS': 'Kansas', 'KY': 'Kentucky', 'LA': 'Louisiana', 'MA': 'Massachusetts', 'MD': 'Maryland', 'ME': 'Maine', 'MI': 'Michigan', 'MN': 'Minnesota', 'MO': 'Missouri', 'MS': 'Mississippi', 'MT': 'Montana',
...
def reset_database():
    ...
    c.execute(
        "CREATE TABLE IF NOT EXISTS Users(username TEXT, password TEXT, userID INTEGER);")
    c.execute("CREATE TABLE IF NOT EXISTS Forum(postID INTEGER, parentID INTEGER, username TEXT, text TEXT, date TEXT);")
    c.execute(
        "CREATE TABLE IF NOT EXISTS States(state TEXT, state_short TEXT, happiness REAL, wage REAL);")
...
def print_all_states():
    # open if file exists, if not it will create a new db
    db = sqlite3.connect(DB_FILE)
...
def get_total_broken_by_state(state):
    return len([i for i in MCBROKEN if i["properties"]["is_broken"]
                 and i["properties"]["is_active"] and i["properties"]["state"] == state])

def get_ratio_broken_by_state(state):
    broken = get_total_broken_by_state(state)
    total = get_total_tracked_locations_by_state(state)
    if total == 0:
        return None
    return broken/(total)
...
def get_state_stats(state):
    ...
    try:
        return {
            ...
            "broken": get_total_broken_by_state(state),
            "locations": get_total_tracked_locations_by_state(state)
        }
    except:
        return {
            "name": state_name,
